...
if(isset($_GET['err']) || isset($_GET['pwd'])) {
    if($_GET['err'] === 'pwdnotmatch') {
        echo '<script>alert("As senhas não as mesmas");</script>';
...
<div class="login-form mt-4" style="height: 300px;">
    <h1 class="text-primary mb-3 text-center">Redefinir Senha</h1>
    <?php 
    $selector = $_GET['selector'];
    $validator = $_GET['validator'];    
    if(empty($selector) || empty($validator)){
        // echo $_GET;
        echo '<script>alert("Não foi possível validar sua solicitação")</script>'; 
    } else {
